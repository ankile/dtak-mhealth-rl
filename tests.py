...
def reward_radius(height, width, reward_distance, cost_radius, reward_mag, cost_mag):
    ...
    for x in range(reward_distance - cost_radius, reward_distance + cost_radius + 1):
        for y in range(reward_distance - cost_radius, reward_distance + cost_radius + 1):
...
def immediate_vs_jackpot(height, width, reward_idx1, reward_idx2, reward_mag1, reward_mag2):
    reward_dict = {}
    return
...
length = 10
test = Experiment_1D(length, default_prob, rewards_dict=latent_cost(length, reward_mag=20, cost_mag=-1))
...
test.mdp.solve(setup_name=setup_name, policy_name="Reward Agent: reward value={}".format(reward_agent_R_val))
test.mdp.reset()
