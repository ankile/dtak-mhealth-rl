...
base = Experiment_1D(
    length, make_right_prob=default_prob, rewards_dict=rewards_dict, gamma=default_gamma
)  # has 100% confidence
value_base, policy_base = base.mdp.solve(
    setup_name=setup_name,
    policy_name="Custom Agent: \u03B3={:.3f}, p={:.3f}".format(
        default_gamma, default_prob
    ),
    heatmap=False,
)
...
for gamma in gammas:
    ...
    for prob in probs:
        test = Experiment_1D(
            length, make_right_prob=prob, rewards_dict=rewards_dict, gamma=gamma
        )
        value_test, policy_test = test.mdp.solve(
            setup_name=setup_name,
...
hmap = sns.heatmap(
    ...
    cbar=True,
    cbar_kws={"label": "MSE"},
    annot_kws={"size": 35 / np.sqrt(len(mses_df))},
)
