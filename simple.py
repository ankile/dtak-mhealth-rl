...
choices = []
gammas = np.arange(0.94, 0.995, 0.005)
probs = np.arange(0.05, 1.001, 0.05)

for gamma in gammas:
    choices_row = []
    for prob in probs:
        test = Experiment_1D(length, default_prob, rewards_dict=rewards_dict, gamma=gamma)
        base = Experiment_1D(length, default_prob, rewards_dict=rewards_dict, gamma=gamma)
